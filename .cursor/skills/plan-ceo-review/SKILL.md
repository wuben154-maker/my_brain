...
1. **Re-ground:** State the project, the **current branch or workspace context** (from the host / user if needed — do not guess from stale chat), and the current plan/task. (1-2 sentences)
2. **Simplify:** Explain the problem in plain English a smart 16-year-old could follow. No raw function names, no internal jargon, no implementation details. Use concrete examples and analogies. Say what it DOES, not what it's called.
3. **Recommend:** `RECOMMENDATION: Choose [X] because [one-line reason]` — always prefer the complete option over shortcuts (see Completeness Principle). Include `Completeness: X/10` for each option. Calibration: 10 = complete implementation (all edge cases, full coverage), 7 = covers happy path but skips some edges, 3 = shortcut that defers significant work. If both options are 8+, pick the higher; if one is ≤5, flag it.
4. **Options:** Lettered options: `A) ... B) ... C) ...` — when an option involves effort, show both scales: `(human: ~X / agent-assisted: ~Y)`
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is small with an AI coding agent. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.

**Effort reference** — always show both scales:

| Task type | Human team | Agent-assisted | Compression |
|-----------|-----------|----------------|-------------|
...
Before building anything unfamiliar, **search first** (WebSearch, docs, issue trackers).
- **Layer 1** (tried and true) — don't reinvent. **Layer 2** (new and popular) — scrutinize. **Layer 3** (first principles) — prize above all.

**Eureka:** When first-principles reasoning contradicts conventional wisdom, **name it explicitly in the review output** so the team can discuss or document it. No external logging step is required.
...
## Plan Status Footer (plan mode only)

When you are in **plan mode** and about to finish:

1. If the plan file already has a `## PLAN REVIEW REPORT` section with fresh data from this run, you may leave it; otherwise update or append it.
2. **Do not** depend on external review-dashboard binaries. Summarize review status **from this session's findings**.

Append or replace a section titled exactly `## PLAN REVIEW REPORT` at the **end** of the plan file. Use this template (fill in from this CEO review; for rows not yet run, use `—`):
...
|--------|---------|-----|--------|----------|
| CEO Review | `/plan-ceo-review` | Scope & strategy | {e.g. COMPLETE / —} | {short summary} |
| Eng Review | `/plan-eng-review` | Architecture & tests | {—} | {—} |
| Design Review | `/plan-design-review` | UI/UX | {—} | {—} |
| Outside voice | optional independent reviewer | Blind spots | {—} | {—} |

**VERDICT:** {e.g. CEO review done — run `/plan-eng-review` before implementation if engineering sign-off is required.}
```

## Merge base (for comparing change scope)

Determine **which branch this work is targeting** (merge base) so you can reason about **what already changed vs the base**. Use whatever the **host UI**, **user**, or **attached PR metadata** provides — ask the user if unknown. A common default name is `main`; treat any name the team uses the same way.

When you compare plans to the codebase, interpret **`<base>`** in this skill as that merge-base branch name — discover it only from the host, user, or PR metadata, never by running commands.
...
* COMPLETENESS IS CHEAP: AI-assisted coding compresses implementation time 10-100x. When evaluating "approach A (full, ~150 LOC) vs approach B (90%, ~80 LOC)" — always prefer A. The 70-line delta is cheap with an agent. "Ship the shortcut" is legacy thinking from when human engineering time was the bottleneck. Boil the lake.
...
Before doing anything else, gather **repo context**. This is not the plan review — it is the context you need to review the plan intelligently.

Use **Read**, **Glob**, and **Grep** only: open **CLAUDE.md**, **AGENTS.md**, **TODOS.md**, **README**, and **docs/** (architecture / designs) if present; scan for **FIXME** / **TODO** / **HACK** / **XXX** in files relevant to the plan (skip giant vendored trees).

**Design doc discovery (no external tooling required):** use **Glob** / **Grep** / **Read** on the repo. Typical locations: `docs/designs/`, `docs/`, `design/`, or paths the user pasted. Filenames may include `design`, `prd`, `spec`.

If a design doc exists (often produced after `/office-hours` or by the team), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note:** if you find `*ceo-handoff*.md` or the user attaches a handoff path, read it. Use it to avoid re-asking answered questions.

Tell the user when you use it: "Using your prior handoff / design note for context."

## Prerequisite Skill Offer

When **no design or problem-framing doc** turns up in the repo (and the user did not paste one), offer the prerequisite skill before proceeding.
...
After /office-hours completes, **re-scan the repo** for a new design doc (same discovery rules as above).
...
After completion, **re-scan the repo** for a new design doc and resume the review.
...
* What is already in flight (other open PRs, branches, or WIP the user described)?
* What are the existing known pain points most relevant to this plan?
* Are there any FIXME/TODO comments in files this plan touches?

### Retrospective Check
If commit history or PR descriptions are visible in the host (or the user summarizes them), look for prior review cycles (review-driven refactors, reverts). Note what changed and whether the current plan re-touches those areas; be **more** aggressive where problems recurred.
...
Use the **Search Before Building** framework from this skill's preamble. Before challenging scope, understand the landscape. WebSearch for:
...
Feed into the Premise Challenge (0A) and Dream State Mapping (0C). If you find a eureka moment, surface it during the Expansion opt-in ceremony as a differentiation opportunity, and **name it in the written review** (see Eureka in Search Before Building).
...
**Default path (in the repo):** create `docs/ceo-plans/` if needed. If the team uses a different location, use that instead.

Before writing, check for existing CEO plans in that directory. If any are stale (>30 days old or branch merged), offer to move them under `docs/ceo-plans/archive/`.

Write to `docs/ceo-plans/{YYYY-MM-DD}-{feature-slug}.md` (or your team's chosen path) using this format:
...
3. **Metrics:** include iteration count, issues found/fixed/remaining, and quality score in your summary to the user. No separate log file is required.
...
```
NOTE: These represent human-team implementation hours. With an AI coding agent,
6 hours of human implementation often compresses to ~30-60 minutes. The decisions
are identical — only the execution speed changes. Always present both scales when discussing effort.
...
different model or session. Two independent passes on a plan are stronger signal than one.

Use AskUserQuestion:

> "All review sections are complete. Want an outside voice? Another model (fresh session) can
...
the user pointed this review at, or the scope the user described). If a CEO plan document
...
**How to obtain the outside voice:**
1. **Preferred:** Dispatch via the **Agent tool** a **new** subagent with only this prompt and the **file paths** to read. The subagent must not inherit this chat's earlier reasoning — genuine independence.
2. **Alternative:** Ask the user to run the same prompt in another product or chat and **paste** the reply; treat the paste as the outside voice.

Present the reply under a header such as `OUTSIDE VOICE (independent):` — **verbatim**, no trimming.

If subagent dispatch is unavailable and the user does not paste a review: "Outside voice unavailable. Continuing to outputs."
...
**Persist the result:** note in the session summary whether outside voice ran (subagent vs user-pasted), and whether substantive findings were produced.
...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Use the project's **TODOS.md** or team template if one exists.
...
* **Effort estimate:** S/M/L/XL (human team) → with agent assist: compress roughly one bucket (e.g. M→S, L→M)
* **Priority:** P1/P2/P3
...
```
  ...
  | Outside voice        | ran (subagent / user paste) / skipped        |
...
## Handoff / draft cleanup (optional)

If you created **temporary handoff files** in the repo during the review, offer to delete or archive them after the Completion Summary. Do not delete anything the user still wants to keep.

## Review Readiness summary (conversation output)

After the Completion Summary, print a **lightweight** readiness table for the **human reader**. Do not pipe this through external tools — derive cells from **this session** and what the user told you about prior reviews (if anything).

Example shape (fill honestly; use `—` when unknown):

```
+====================================================================+
|                 REVIEW READINESS (THIS SESSION)                     |
+====================================================================+
| Review          | Status this run | Notes                           |
|-----------------|-----------------|---------------------------------|
| CEO Review      | DONE            | {mode + headline outcome}       |
| Eng Review      | —               | recommend /plan-eng-review next  |
| Design Review   | —               | if UI scope                     |
| Outside Voice   | —               | optional                        |
+====================================================================+
```

**Review tiers (guidance):**
- **Eng Review:** Recommend `/plan-eng-review` when architecture, test strategy, or delivery risk needs a second pass. Whether it "gates" shipping is a **team policy** — this skill does not enforce tooling.
- **CEO Review:** Optional but valuable for big product bets and scope decisions.
- **Design Review:** Optional; use `/plan-design-review` when UI/UX intentionality matters.
- **Adversarial / pre-landing review:** Follow your team's practice (if any).

**Staleness:** If the user mentions a prior review from an older commit, suggest re-running when scope or architecture moved.

## Plan File Review Report (plan mode)

When there is an **active plan file** for this session, mirror the **Plan Status Footer** template: ensure `## PLAN REVIEW REPORT` at the **end** of that file reflects this CEO review (see earlier in this skill).

- Search for `## PLAN REVIEW REPORT` anywhere in the file; **replace** from that heading through the next `## ` heading or EOF.
- If missing, **append** at EOF.
- Use the same table column set as in **Plan Status Footer** (CEO / Eng / Design / Outside voice + VERDICT). Fill from **this run** — do not require JSONL or dashboard binaries.

## Next Steps — Review Chaining

After the **Review Readiness summary**, recommend the next review(s) based on what this CEO review discovered.

**Usually recommend `/plan-eng-review` next** when the plan will drive implementation — especially if scope expanded, architecture shifted, or risk is non-trivial. Skip only when the user says eng review is unnecessary for this change.

**Recommend `/plan-design-review`** when Section 11 applied (UI scope) or when accepted expansions are user-facing.

**If both apply, recommend eng review first**, then design review.

Use AskUserQuestion to present the next step. Include only applicable options:
- **A)** Run /plan-eng-review next (recommended before implementation for most non-trivial plans)
...
- **B)** Keep under `docs/ceo-plans/` (or your draft folder) without promoting to `docs/designs/`
- **C)** Skip
