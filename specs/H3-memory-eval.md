...
- **阶段：** 硬化项 · **执行时机：** **起步跟 M1**（召回可测后），**加强跟 C3**（自进化曲线需纵向数据）· **状态：** 📝 待做
...
```
- 阈值化：`recall@5 >= 阈值`、`evolution 曲线非降` 等写成断言；阈值进 spec，调整需改 spec（棘轮）。
- 输出：评测报告（命中率/曲线）可打印；可选接入 H0 覆盖率/CI 一起跑。

## 4. 验收清单
- [ ] 固定语料 + mock 引擎下评测可复现、确定性（同输入同结果）。
- [ ] `recall@k` 与"成长曲线非降"指标实现并阈值断言。
- [ ] 阈值不达标时测试**红灯**（验证断言真的会失败）。
- [ ] 不依赖真实网络/key；可在 CI 跑。
