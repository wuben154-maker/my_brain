...
- **阶段：** B · **状态：** ✅ 已实现（`src/agent/jobs/topicResearchJob.ts`、`topicResearchJob.test.ts`）
...
## 4. 验收清单
- [x] 给定 topic + mock 源，产出 ≥2 个互相 `link` 的提议且依赖顺序正确。
- [x] `source==="research_loop"`、全部 `pending`。
- [x] 步数/预算任一超限优雅停止，trace 记录原因。
- [x] 全程无存储写（沿用护栏断言）。
- [x] abort 即停。
