...
## 4. 验收清单
- [x] `decay` 指数半衰正确；`computeSalience` 对命中/确认提升、对久置衰减，确定性可测。
- [x] migration 向后兼容：旧节点无 `salience` 字段也正常加载（默认值）。
- [x] 衰减**只产信号**：无任何自动归档/删除路径（不变量测试断言）。
- [x] C2 能消费 salience 排序出"低显著度 + 久未触达"候选（接口对齐 C2）。
- [x] （SHOULD）星图低显著度节点视觉弱化截图。
