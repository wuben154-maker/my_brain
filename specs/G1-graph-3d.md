...
- **阶段：** A 段收尾 / B 段（建议 A4 之后、B3「星图预览高亮」之前或同期）· **状态：** 📝 待做
...
## 5. 验收清单（含 §9 截图反馈闭环）
- [ ] 「3D 视图」按钮切换 2D⇄3D，状态可视且可逆。
- [ ] 3D 下鼠标可旋转/缩放/平移；点击节点选中并高亮（与 2D 行为对齐）。
- [ ] A4/B3 的提议「预览高亮」在 3D 视图下也命中正确节点（若 B3 已实现）。
- [ ] 默认与 `?visual=main` 仍是 2D，`visual:*` 像素对比零回归。
