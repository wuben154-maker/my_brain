...
- [ ] App 启动后 `proposalStore.load()` 拉取 pending，铃铛角标数正确。
- [ ] 逐条「同意」后对应节点在星图出现/更新；「拒绝」后消失且不改图谱。
- [ ] 简报卡片可关闭，关闭不持久化原文（符合「原文聊完即丢」）。
- [ ] 浏览器自动化截图「收件箱→确认→星图变化」闭环，并与 `DESIGN.md` 视觉对比（截图反馈闭环）。
- [ ] 空收件箱有合理空态。
