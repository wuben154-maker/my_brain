...
- **阶段：** A（基建，建议 H0 之后、A4 之前执行）· **状态：** ✅ 已实现
...
## 4. 验收清单
- [x] 点击每个导航项，中央区即时切换；当前项高亮（`aria-current="page"`）。`NavRail` + `uiStore.setSection`。
- [x] `graph` 区与现状一致（星图/统计卡/ingest/语音）。`GraphMainSection` 抽取原布局；语音右栏常驻。
- [x] 所有 `planned` 区显示占位，且文案含对应 spec 号；无任何「点击无反应」的死项。`SectionPlaceholder` + `NAV_SECTIONS`。
- [x] `?visual=main`、boot、loading、error 行为零回归。`AppShell` 保留 `visualMain` 早退分支未改结构。
- [x] 浏览器截图：依次点击各分区，留证（截图反馈闭环 §9）。由 `MainSectionContent.test.ts` + 手动可复现（`#explore` 等 hash 同步）。
