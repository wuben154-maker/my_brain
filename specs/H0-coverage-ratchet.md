...
test: {
  ...
  coverage: {
    ...
    thresholds: { lines: 70, functions: 70, branches: 60, statements: 70 }, // 按实测基线设，宁可略低于现状再 ratchet
  },
}
```
...
## 4. 验收清单
- [ ] 本地 `pnpm coverage` 能跑出报告且通过阈值（先把阈值设到≤当前实测值，确保一开始就是绿）。
- [ ] 故意删掉某逻辑文件的测试 → `pnpm coverage` 红（证明棘轮生效）。
- [ ] CI 包含覆盖率步骤，PR 上会跑。
- [ ] GitHub 分支保护已开（截图/设置项确认）：CI 红时合并按钮被禁用。
- [ ] `pnpm check`（本地快闸门）仍不含 coverage，保持快。
