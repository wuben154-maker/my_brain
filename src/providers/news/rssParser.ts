...
/** Minimal RSS/Atom parser — no external XML dependency. */
  ...
  if (/<entry[\s>]/i.test(xml)) {
    ...
    for (const entry of entries) {
      ...
      const link =
        ...
        firstTag(entry, "link");
      const summary =
        stripHtml(firstTag(entry, "summary")) ||
        stripHtml(firstTag(entry, "content"));
      const publishedAt =
        firstTag(entry, "updated") || firstTag(entry, "published") || null;
      if (!title || !link) {
        continue;
      }
      items.push({
