...
function hasField(text: string, patterns: RegExp[]): boolean {
  return patterns.some((pattern) => pattern.test(text));
}

function parseKnowledgeOsStatus(text: string): ParsedReport["knowledgeOsStatus"] {
...
function parseSmokeRecords(text: string): Array<Record<string, string>> {
      ...
      if (Array.isArray(parsed)) {
        for (const item of parsed) {
          if (item && typeof item === "object") {
            records.push(item as Record<string, string>);
          }
        }
      } else if (parsed && typeof parsed === "object") {
        records.push(parsed as Record<string, string>);
      }
    } catch {
...
function parseM2Keys(text: string): string[] {
  const keys = [
    "migration_gate",
...
function parseReportLinks(text: string): string[] {
...
function parseExpoEvidence(text: string): boolean {
...
export function parseGateReport(path: string, raw: string | null): ParsedReport {
  if (!raw) {
    return {
      path,
      exists: false,
