...
import { parseGateReport } from "./report-parser.ts";
import {
  requirePath,
  requireScript,
  runCommand,
...
import type { CheckResult, GateContext, ParsedReport, StageId, Verdict } from "./types.ts";
import {
...
function push(results: CheckResult[], check: CheckResult): void {
  results.push(check);
}

function worstVerdict(checks: CheckResult[]): Verdict {
...
function allowedActionFor(stage: StageId): string {
  if (stage === "M7") {
    return "complete";
  }
...
function loadReport(root: string, stage: string): ParsedReport {
  const rel = reportPathForStage(stage);
  return parseGateReport(rel, readText(resolveFromRoot(root, rel)));
}

function checkRequiredInputs(ctx: GateContext, results: CheckResult[]): void {
  const required = [
...
function detectPipelineOverreach(ctx: GateContext, results: CheckResult[]): void {
...
function checkM7Sequence(ctx: GateContext, results: CheckResult[]): void {
...
function checkSequence(ctx: GateContext, results: CheckResult[]): void {
  if (ctx.stage === "M7") {
...
function checkReportFields(ctx: GateContext, results: CheckResult[]): void {
  if (!ctx.parsedReport?.exists) {
    push(results, {
      id: "report-missing",
      category: "report",
  ...
  const fieldEntries = Object.entries(ctx.parsedReport.fields) as Array<
    [string, boolean]
  >;
...
function grepFixtureIds(filePath: string, fixtureIds: readonly string[]): string[] {
...
function checkM0Commands(ctx: GateContext, results: CheckResult[]): void {
...
function checkM1Commands(ctx: GateContext, results: CheckResult[]): void {
  const paths = [
  ...
  if (fileExists(coldFixtures)) {
    const data = readJson(coldFixtures) as { fixtures?: unknown[] } | null;
  ...
  if (fileExists(captureFixtures)) {
    const data = readJson(captureFixtures) as { fixtures?: unknown[] } | null;
    const count = Array.isArray(data?.fixtures) ? data!.fixtures!.length : 0;
    push(results, {
...
type M2IosBackupEvidenceFileEntry = {
  path?: string;
  exists?: boolean;
...
type M2IosBackupEvidenceArtifact = {
  deviceEvidence?: string;
  platform?: string;
  checkedAt?: string;
  files?: M2IosBackupEvidenceFileEntry[];
};
...
function validateM2IosBackupEvidenceArtifact(data: unknown): {
  ok: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  const artifact = data as M2IosBackupEvidenceArtifact;
  ...
  for (const suffix of [".db", "-wal", "-shm"] as const) {
...
function loadM2IosDeviceEvidence(root: string): {
  present: boolean;
  errors: string[];
} {
...
function checkM2Commands(ctx: GateContext, results: CheckResult[]): void {
  const tests = [
...
function checkM3Commands(ctx: GateContext, results: CheckResult[]): void {
  const rootPkg = resolveFromRoot(ctx.root, "package.json");
...
function checkM4Commands(ctx: GateContext, results: CheckResult[]): void {
...
function checkM6Commands(ctx: GateContext, results: CheckResult[]): void {
...
function checkM7ACommands(ctx: GateContext, results: CheckResult[]): void {
  const fixtures = [
...
function checkM7BCommands(ctx: GateContext, results: CheckResult[]): void {
...
function checkM7Commands(ctx: GateContext, results: CheckResult[]): void {
  checkM7Sequence(ctx, results);
}

function checkStageCommands(ctx: GateContext, results: CheckResult[]): void {
  const rootPkg = resolveFromRoot(ctx.root, "package.json");
...
export function runStageChecks(ctx: GateContext): {
  checks: CheckResult[];
  verdict: Verdict;
} {
...
export function buildContext(root: string, stage: StageId): GateContext {
...
export function printResult(stage: StageId, checks: CheckResult[], verdict: Verdict): void {
  const categories = ["sequence", "report", "commands", "forbidden", "deviceEvidence"] as const;
  const categoryVerdict = (category: (typeof categories)[number]): Verdict => {
    const subset = checks.filter((check) => check.category === category);
    return subset.length > 0 ? worstVerdict(subset) : "PASS";
  };
