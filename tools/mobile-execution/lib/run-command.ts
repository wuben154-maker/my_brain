...
export interface RunCommandOptions {
  id: string;
  cwd?: string;
  hardStopOnFailureWhenReportPass?: boolean;
  reportClaimsPass?: boolean;
  timeoutMs?: number;
}

function resolveTimeoutMs(command: string, explicit?: number): number {
...
export function shouldExecuteCommands(): boolean {
...
function splitCommandArgs(argsString: string): string[] {
  if (!argsString.trim()) {
    return [];
  }
  const args: string[] = [];
...
function pnpmCjsCandidates(): string[] {
...
function findPnpmCjs(): string | null {
...
export function resolveCommandInvocation(command: string): ResolvedInvocation {
...
function enrichPathForPnpm(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
